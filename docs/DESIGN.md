...
| 站点图标是一个狗脸（🐶） | v4 那版是蓝色渐变圆角方块 + 字母 Y，属于已经被取消的强调色，在现在的单色主题里是唯一的彩色遗留。图形取自 Twemoji 的 U+2615（CC-BY 4.0），矢量 + 一整套光栅（ico 16/32/48、png 16/32、apple-touch 180），顺序照参考站。不用「SVG 里放 emoji 文本」那种做法：那要靠用户系统的彩色 emoji 字体，渲染结果不可控，而且彩色的 emoji 字体字形不能随便再分发。图标文件放 `src/assets/` 走 import 拿内容哈希，因为 `/favicon.svg` 这种固定路径会被浏览器长期缓存，上一版就因为缓存怎么改都看不到 |
