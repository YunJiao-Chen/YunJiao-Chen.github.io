...
| 设计规范 | 视觉 = PaperMod 原样，流程按 [`design-taste-frontend`](https://github.com/Leonxlnx/taste-skill)；54 项可执行自检 + 主题哈希校验，都进 CI |
...
```bash
npm run build && npm run audit   # 54 项断言，全部通过才允许发布
```
...
├─ scripts/
│  ├─ taste-audit.mjs        # 设计规范自检（54 项：对比度 / 文案 / 结构）
...
| `npm run audit` | 设计规范自检（对比度 / 文案 / 结构共 54 项，接 CI 可当门禁） |
| `npm run preview` | 预览 `dist/` |
